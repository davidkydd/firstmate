...
- **Optional Microsoft Teams bridge** - deploy an approved single-tenant bot and Azure queue to submit status and allowlisted read-only mobile requests through an outbound-only Mac connector, with durable identity binding and local confirmation required for every other action.
