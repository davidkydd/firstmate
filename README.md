...
- **Optional Microsoft Teams bridge** - deploy an approved single-tenant bot and Azure queues to submit status and general mobile requests through an outbound-only Mac connector, with durable identity binding and exact trusted-local approval before request text reaches normal intake.
...
- [docs/teams-integration.md](docs/teams-integration.md) - current setup, formal Torus rollout, certificate rotation, monitoring, privacy, retention, and rollback for the disabled-by-default Teams bridge.
- [docs/teams-architecture.md](docs/teams-architecture.md) - maintainer architecture for Teams identity binding, the typed queue protocol, durable replay, authority limits, and exact reply correlation.
