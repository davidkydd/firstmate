...
- **Optional Copilot app surface** - explicitly install a user-scoped `/firstmate` command and fleet canvas that replay exact external Firstmate outcomes across app-session replacement without making the app or its local state authoritative.
...
- [docs/copilot-app-surface.md](docs/copilot-app-surface.md) - setup, replay, provenance, capability checks, and safety limits for the optional GitHub Copilot app surface.
