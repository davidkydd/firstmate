...
- **Optional Microsoft Teams bridge** - deploy an approved single-tenant bot and Azure queue to submit status and general mobile requests through an outbound-only Mac connector, with durable identity binding and exact trusted-local approval before request text reaches normal intake.
