...
Shared tracked material is `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, public `skills/`, and `fleet/`.
...
fleet/               tracked reusable persistent-role definitions; fleet/README.md and bin/fm-fleet-validate.sh own their contract
...
config/remote-transports  optional primary-local SSH-alias transport profiles for remote secondmates; LOCAL, gitignored, and NOT inherited; absent leaves generic SSH unchanged; see docs/configuration.md "Remote transport profiles"
...
config/teams.json  optional owner-only single-tenant Teams connector configuration; LOCAL, gitignored, disabled by absence or enabled=false, and not inherited; see docs/teams-integration.md
...
state/               runtime records and signals; gitignored
  ...
  <id>.write-boundary  private token-bound source-checkout/worktree identity for ship and scout pre-tool write protection; bin/fm-spawn.sh creates it and docs/crew-primary-write-guard.md owns the contract
  ...
  inbox/             captain notes captured out of band by bin/fm-inbox.sh, including the voice handover's queued requests and idempotent Teams transport requests; each note appends one `check` wake and stays pending until acknowledged with `bin/fm-inbox.sh drain --ack <id>`, which moves it to inbox/handled/ (docs/voice-relay.md; docs/teams-architecture.md)
  teams/             private Teams request/result correlation records written only by the outbound connector; a spawned task may carry its request binding as `teams_request=` in task metadata (docs/teams-architecture.md)
...
Tracked named-role definitions under `fleet/agents/` may supply reusable charter, scope, asset, periodic-work, and external-write policy while each private home retains its own paths, projects, credentials, and durable work; `fleet/README.md` owns that boundary.
...
- `teams-respond` - load on an inbox notification whose note ID begins `external-teams-`, and before reporting a milestone or terminal outcome for a task whose metadata carries `teams_request=`; relevant only when the private Teams connector is enabled.
