...
The optional user-scoped GitHub Copilot app extension stores its transport under `state/captain-surfaces/` in one explicitly selected `FM_HOME`.
[`copilot-app-surface.md`](copilot-app-surface.md) owns current setup, supported limits, replay behavior, and uninstall.
`bin/fm-captain-surface.sh` owns the append-only journal, generation, cursor, provenance, and typed-application formats.
The extension's generated `config.json` is app-local installation metadata rather than a Firstmate `config/` setting, contains no credential, and never becomes fleet truth.
