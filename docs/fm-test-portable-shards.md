...
The 2026-09-21 local focused runs `bin/fm-test-run.sh tests/fm-crew-primary-write-check.test.sh`, `tests/fm-persistent-roles.test.sh`, `tests/fm-gemini-harness.test.sh`, `tests/fm-review-watch-triage.test.sh`, and `tests/fm-pr-state.test.sh` supplied the initial 17000, 79000, 3517, 587, and 7449 ms hints for those newly measured scripts.
Two fork-only scripts use their successful per-script records from [run 35562445462](https://github.com/davidkydd/firstmate/actions/runs/35562445462) on 2026-09-21: `tests/fm-pr-poll-ado.test.sh` at 1153 ms and `tests/fm-scm-lib.test.sh` at 723 ms.
That partial run's failed `tests/fm-claude-config-seed.test.sh` invocation is excluded because failed invocations are not healthy duration samples.
Taking the slowest of several CI runs rather than a single run keeps the balance honest on a slow runner.
A focused local run on 2026-09-21 used `bin/fm-test-run.sh tests/fm-ci-workflow.test.sh tests/fm-nm-test-contract.test.sh tests/fm-stat-shadowing.test.sh --jobs 1 --json /tmp/fm-hints.json` and measured 2586 ms, 654 ms, and 2234 ms respectively; those current hints keep the unmeasured-share guard below its refusal threshold until broader green CI artifacts supersede them.
The same integration run measured `tests/fm-teams-integration.test.sh` at 13837 ms after its fixture-backed queue and inbox checks were added.
